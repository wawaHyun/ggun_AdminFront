    ...
    <div className="h-screen w-screen flex justify-center">
      <div className="h-full w-[80%] sticky ">
        ...
        <div className="mb-5 ">
          ...
          <Link href={mainNews.imgLink}>
            <div className="flex border bg-gray-200 text-center text-black hover:text-gray-500 rounded-lg">
              <a><Image unoptimized src={mainNews.imgSrc} height={250} width={500} alt={mainNews.title} className="rounded-l-lg" /></a>
              <ul className="w-[50%] ml-[15px] text-center content-center">
                <li className="text-[20px] text-orange-500 ml-[15px] text-left mt-[10%]">경제</li>
                <li className="text-[30px] ">{mainNews.title}</li>
                <li className="text-[15px] ml-[15px] text-left mt-[10%]">{mainNews.content}</li>
              </ul>
            </div>
          </Link>
        </div>
          ...
          {newslist.map((v: INews, i: number) =>
            i < 5 ?
              <ul key={v.id} className="border p-2 w-1/5 text-center text-black hover:text-gray-500 hover:shadow-lg hover:border rounded-lg">
                ...
                <li className="text-left text-gray-400 h-[10%]">{i}분전</li>
              </ul> 
              : <div key={v.id}></div>
          )}
        </div>
