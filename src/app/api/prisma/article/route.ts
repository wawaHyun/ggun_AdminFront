...
export async function allArticleListP() {
    const response = await client.articles.findMany({})
    return response
}


export async function limitArticleListP() {
    const response = await client.articles.findMany({
        take: 30,
    })
    return response
...
export async function findArticleListByIdP(paramsId: string) {
    const board = parseInt(paramsId)
    const response: IpArticle[] = await client.articles.findMany({
        where: {
            board_id: board
        },
...
export async function findArticleByIdP(id: number) {
...
export async function updateAricleP(aricle: IpArticle) {
